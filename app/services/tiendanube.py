...
class TiendaNubeClient:
    ...
    def parse_tracking_file(self, file_content) -> list:
        """
        Parses the file (PDF/Excel/CSV) and returns a normalized list of tracking items.
        Returns list of dicts: {"order": "123", "track": "ABC"}
        """
        # Determine file type
        try:
            if file_content.startswith(b"%PDF"):
                 ...
                 if df.empty:
                     # Instead of error, we can return empty list or raise custom
                     raise ValueError("No valid labels found in PDF. Could not identify 'Interno.'")
            else:
                try:
        ...
        if not 'order' in df.columns:
            ...
            if not col_order or not col_track:
                raise ValueError("Could not identify columns (expecting 'orden'/'numero' and 'seguimiento'/'track').")
        ...
        for _, row in df.iterrows():
            ...
            if not order_number or not track_code or track_code.lower() == "nan":
                 continue # Skip empty rows silentely or maybe log?
    ...
    def process_single_tracking_item(self, item: dict) -> dict:
        """
        ...
        try:
            ...
            if status_code in [200, 201]:
                ...
                return {"order": order_number, "status": "ERROR", "details": details}
                
        except Exception as e:
            return {"order": order_number, "status": "EXCEPTION", "details": str(e)}

    def process_tracking_file(self, file_content):
        # Legacy Wrapper for non-batched calls (to be deprecated or kept as simple wrapper)
        try:
            items = self.parse_tracking_file(file_content)
        except Exception as e:
            return {"error": str(e)}

        results = []
        for item in items:
            results.append(self.process_single_tracking_item(item))
